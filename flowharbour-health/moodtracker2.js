...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Define messages for each button (btn1 to btn5)
    const messages = {
        happy: "Your happiness is like sunshine—it lights up everything around you! 😊 Keep shining bright!",
        sad: "Even on cloudy days, remember that the sun is still shining behind the clouds. You’ll feel better soon! 💛",
        angry: "Take a deep breath and let go of the storm inside. You’ve got this! 🌟",
        tired: "Rest up, recharge, and take it easy. Tomorrow is a fresh start! 🌙💤",
        excited: "You're doing your best. Take it slow and be kind to yourself. 💫"
    };
    ...
    if (savedDate === today) {
        ...
        if (savedMood) {
            todayMood.innerHTML = `${savedMood}`; // Display the saved mood
    ...
    saveButton.addEventListener('click', () => {
        ...
        // Display the specific message for the selected mood after saving
        const output = document.getElementById('output');
        output.textContent = messages[selectedMood]; // Display the message based on selected mood
    });
});
